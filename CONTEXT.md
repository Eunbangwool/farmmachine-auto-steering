...
## AGMO Ver2 CAN — 무회전 진단: CAN 미개통 의심 + 개통 code 스윕 (2026-06-22)
실차 logcat 결과: binder 연결 OK, registerCallback(code16) ret=true·서비스 안 죽음. 그러나 RX 등록 후 ~23초간 `code=19`(MCU→콜백) 프레임 **0건**. 모터 전원 시 Keya HB(0x07000001)가 버스에 자동으로 흐르므로 RX 0건 = **CAN 채널 미개통** 강한 신호(autokit2 의 setCANBaudrate 미호출). TX `ret=true`(code7)여도 닫힌 채널이라 프레임 폐기로 추정.
- **추가(진단)**: `CpdeviceCanBridge.openCanSweep()` — RX 콜백 등록 후 transact code 2..24(1/7/16/19 제외)에 `(ch,baud)`/`(baud)` 2형태 송신, 직후 RX 증가분 측정. **RX(모터 HB)가 처음 살아나는 code = CAN 개통 code.** self-recovery 로 서비스 죽어도 재연결. UI "CAN 개통 코드 스윕" 버튼 + `JsBridge.cpdevOpenSweep`.
- 다음(현장): 스윕으로 개통 code 확정 → 그 code 를 start() 에서 1회 호출하도록 고정 → TX(조향) 재시도. 또는 역분석으로 setCANBaudrate code 입수 시 스윕 생략.

## AGMO Ver2 CAN — BnMcuCanService 트랜잭션 코드 확정 (2026-06-22)
역분석(libcpcomm.so Bp 메서드 디스어셈블)로 코드 맵 **확정**. 추측 종료.
- code 1 registerCallback(IBinder) / 2 unregisterCallback / **3 setCANBaudrate(int ch, uint baud, uint baud2) ★CAN 열기** / 4 addFilter / 5 setFilters / 6 clearFilters / 7 sendCanFrame(int N, byte[N×14]) / 8 requestValue / 9 getMcuVersion / 14 getInputStatus / 15 setOutputStatus. RX 콜백은 등록 IBinder 로 역transact(code 는 콜백쪽 구현, 고정 아님).
- **이전 추측 전부 오류 정정**: code1=getMcuVersion(X)→registerCallback, registerCallback=16(X)→1, RX=19(X)→미고정.
- **DeadObject 원인 규명**: setCANBaudrate(code3) 미호출=CAN 미개통 상태에서 sendCanFrame(code7) → 서비스가 핸들 무효화. autokit2 는 시작 시 setCANBaudrate 선행.
- **수정(CpdeviceCanBridge)**: 시작 순서 `getService → setCANBaudrate(code3, ch,baud,baud2) → registerCallback(code1) → sendCanFrame(code7)` 고정. `canOpened` 게이트(미개통 시 자동 개통 선행). DeadObject 시 재연결+개통부터 재실행. 단일 binder 핸들 일원화. setCANBaudrate=oneway(flags=1). RX onTransact 는 고정 code 가정 없이 프레임형 파싱.
- 기본값 ch=0 baud=250000(baud2 동일). 무반응 시 UI 에서 ch/baud(500k) 바꿔 `cpdevOpenCan` 재시도. UI: "CAN 개통(code3)"+baud 250k/500k 버튼, RX 등록(code1). 스윕은 불필요화(백업 유지).

## AGMO Ver2 CAN — 원인 정정: 서비스를 죽이는 건 registerCallback(code1) (2026-06-22)
실차 logcat 재분석으로 원인 정정. **setCANBaudrate(code3)는 무해**(호출 후 467ms binder 생존). 서비스를 죽이는 건 **registerCallback(code1)** — 우리 콜백 IBinder 인터페이스가 미확정이라 호출 직후 ~100ms 에 native cpcomm_server 크래시. PR#8에서 넣은 "재연결 시 콜백 자동 재등록"이 reconnect→register→crash→reconnect 죽음의 폭주를 만들어, init 가 반복크래시로 cpcomm_server 를 **영구 비활성**(이후 getService=null 무한). 버스트(50ms)와 겹쳐 악화.
- **수정**: registerCallback 자동호출 전면 제거(ensureBinder 재등록 삭제, registerRx 기본 OFF). initCan=setCANBaudrate(code3)만. **모터 제어는 sendCanFrame(code7) 단독으로 가능**(RX 피드백 불필요). RX(code1 콜백)는 콜백 인터페이스 역분석 후 별도. UI RX 버튼 "⚠크래시 위험" 경고.
- ★ 서비스 영구 비활성 상태면 **태블릿 재부팅** 필요(init 가 cpcomm_server 재기동).
- 다음(현장): 재부팅 → Ver2 선택(code3만, 크래시 없어야) → Enable+버스트로 cmd_speed 전송 → 모터 회전 확인.
