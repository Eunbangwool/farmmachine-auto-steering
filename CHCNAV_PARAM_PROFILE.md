...
**★ 이식 완료(2026-06-30)**: 위 복원 로직을 `rtk-leveling/src/proportional_valve.py` 로 자체 구현
(leveler_core 미수정 add-on, LevelerOutput **방식 C** = 비례밸브 직결).
- `ChcnavBladeProfile`: PWM 10~96% / 다단계 데드존 0.0001~0.07m / min_speed HOLD
- `ProportionalValveController`: PID + 다단계 데드존 → PWM 듀티 (차속·데드존 HOLD 게이트)
- `SignalFilter`(칼만+이동평균), `IC100Calibration`(4파라미터, IC100Paras.txt %.3lf 로드/저장)
- `ProportionalValveOutput`(듀티→CAN), SITL 폐루프 검증 7/7.
- ★현장튜닝: PID 게인/min_speed/kalman_r 는 .so 미기재 → 추정값(현장 보정 필요).
