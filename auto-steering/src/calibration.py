...
class DualMountDiagnostic:
    """
    듀얼안테나 base/rover · 부호 현장 진단 — 직선 1회 주행으로 자동 추천.

    무빙베이스 RELPOSNED 의 relPosHeading 은 **base→rover 벡터**의 진헤딩이므로,
    직선 주행 중 (relPosHeading vs GPS 진로각) 의 원형평균 = 베이스라인 마운트 오프셋:
      ≈ +90°  → base=좌 / rover=우  → dual_baseline_offset_deg = +90 (현재 코드값)
      ≈ -90°  → base=우 / rover=좌  → -90
      ≈   0°  → 이미 차체헤딩(오프셋 불필요)
      ≈ ±180° → 전후 반전(확인 필요)
    roll 부호: 평지에서 한쪽으로 기울였을 때 relPosD 부호로 dual_roll_sign 추천
      (우측 하강 시 relPosD>0 이면 +1 = 현재 코드값).
    """
    def __init__(self, min_distance_m: float = 15.0, min_samples: int = 60,
                 tilt_thresh_m: float = 0.03):
        self.min_distance = min_distance_m
        self.min_samples = min_samples
        self.tilt_thresh = tilt_thresh_m
        self._sin = 0.0; self._cos = 0.0; self._n = 0
        self._dist = 0.0; self._last: Optional[tuple] = None
        self._baseline_sum = 0.0; self._acc_sum = 0.0
        self._reld_extreme = 0.0                      # 부호 보존 최대 relPosD
        self._logged = False

    def add_sample(self, east: float, north: float, relpos_heading_deg: float,
                   rel_d_m: float = 0.0, baseline_m: float = 1.0,
                   acc_deg: float = 0.0):
        if abs(rel_d_m) > abs(self._reld_extreme):
            self._reld_extreme = rel_d_m
        if self._last is not None:
            de = east - self._last[0]; dn = north - self._last[1]
            step = math.hypot(de, dn)
            if step > 0.05:                           # 충분히 이동(정지 노이즈 제외)
                course = math.degrees(math.atan2(de, dn))     # 나침반(북=0,CW)
                diff = _wrap(math.radians(relpos_heading_deg - course))
                self._sin += math.sin(diff); self._cos += math.cos(diff)
                self._n += 1; self._dist += step
                self._baseline_sum += baseline_m; self._acc_sum += acc_deg
        self._last = (east, north)

    @property
    def progress(self) -> float:
        return min(1.0, min(self._dist / self.min_distance,
    ...
    def report(self) -> dict:
        if self._n == 0:
            return {"ready": False, "note": "샘플 없음 — 직선 주행 필요",
                    "progress": 0.0}
        offset = math.degrees(math.atan2(self._sin, self._cos))
        R = math.hypot(self._sin, self._cos) / self._n
        rec = int(round(offset / 90.0)) * 90          # 가장 가까운 90° 배수
        if rec >= 180 or rec <= -180:
            base, rec_off, interp = "?", 180.0, "전후 반전 — relPosHeading 정의/배선 확인"
        elif rec == 90:
            base, rec_off, interp = "좌측(left)", 90.0, "base=좌 / rover=우 (현재 코드값)"
        elif rec == -90:
            base, rec_off, interp = "우측(right)", -90.0, "base=우 / rover=좌"
        else:  # rec == 0
            base, rec_off, interp = "오프셋불필요", 0.0, "relPosHeading 이 이미 차체헤딩"
        # roll 부호
        if abs(self._reld_extreme) >= self.tilt_thresh:
            roll_sign = 1.0 if self._reld_extreme > 0 else -1.0
            roll_note = (f"기울임 감지 relPosD={self._reld_extreme:+.3f}m → "
                         f"우측하강이었다면 dual_roll_sign={roll_sign:+.0f} "
                         f"(좌측하강이었다면 {-roll_sign:+.0f})")
        else:
            roll_sign = None
            roll_note = "기울임 미감지 — 평지에서 한쪽(우측)으로 기울여 재측정"
        return {
            "ready": self.ready(), "progress": self.progress,
            "n": self._n, "dist_m": round(self._dist, 1),
            "offset_deg": round(offset, 2), "R": round(R, 3),
            "base_antenna": base,
            "rec_baseline_offset_deg": rec_off,
            "rec_dual_roll_sign": roll_sign,
            "baseline_m": round(self._baseline_sum / self._n, 3),
            "acc_heading_deg": round(self._acc_sum / self._n, 3),
            "interp": interp, "roll_note": roll_note,
        }

    def maybe_log(self):
        """ready 가 되는 순간 1회 추천값을 로그로 출력(현장 logcat 확인용)."""
        if self._logged or not self.ready():
            return
        self._logged = True
        r = self.report()
        log.info(f"[듀얼마운트 진단] offset={r['offset_deg']:+.1f}° R={r['R']:.2f} "
                 f"→ base={r['base_antenna']} (dual_baseline_offset_deg={r['rec_baseline_offset_deg']:+.0f}). "
                 f"baseline={r['baseline_m']}m accHeading={r['acc_heading_deg']}°. {r['roll_note']}")
