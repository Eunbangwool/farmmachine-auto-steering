...
- ✅ 가변시비(VRA)+섹션컨트롤(`section_control.py`, 시비·방제 공통): 처방맵(GeoJSON Rx)→위치별 살포율,
  커버리지/경계/헤드랜드/처방0/제외구역 기반 섹션 자동 OFF, 자체 CAN 규약 **FMSC v1**(`IMPLEMENT_CONTROL_PROTOCOL.md`).
  ★ 실 컨트롤러 HW 미보유 → `IMPLEMENT_CAN_VERIFIED=False`(송신차단, 검증 후 True). app_main/JsBridge 호출표면 연동.
...
auto-steering/src/section_control.py        — 가변시비/섹션컨트롤 (FMSC v1 CAN, 처방맵)
